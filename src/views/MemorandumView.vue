...
async function refreshVersionContents(){
    let contentsQuery = encodeURIComponent(`SELECT Id, Name, Order__c, Parent__c, ActiveComments__c, ExternalComments__c, Body__c FROM MemorandumContent__c WHERE Parent__r.Parent__c ='${recordId.value}' ORDER BY Order__c ASC`);
    ...
    try {
        ...
        versionContents.value = contentsResponse.data.records.map((item,index,arr)=>{
            let {Id, Name, Order__c, Parent__c, ActiveComments__c, ExternalComments__c, attributes } = item;
            let isFirst = (index === 0) ? true:false;
            let isLast = (index === (arr.length-1)) ? true:false;
            return {attributes, Id, Name, Order__c, Parent__c, ActiveComments__c, ExternalComments__c, isFirst, isLast};
        });
    } catch(e) {
        handleCalloutException(e);
    }
