...
function setSelectedtocItem(recordId){
    selectedTocItem.value = recordId;
    let foundItem = props.sections.find(item => item.Id === recordId);
    if(foundItem === undefined){
        foundItem = props.contents.find(item => item.Id === recordId);
    }
    emit('selection',{detail:foundItem});
}
</script>
                ...
                <li v-for="tocItem in tocData" v-bind:key="tocItem.Id" aria-level="1" v-bind:aria-selected="tocItem.selected" 
                    ...
                    <div class="slds-tree__item">
                        <button v-bind:class="{'slds-button':true, 'slds-button_icon':true, 'slds-m-right_x-small':true, 'slds-hidden':!tocItem.hasChildren }" 
                            v-bind:aria-hidden="!tocItem.hasChildren" tabindex="-1" title="Expand Tree Branch" v-on:click.self.stop="expandedTocItem = tocItem.Id">
                        ...
                        <span class="slds-has-flexi-truncate">
                            <span class="slds-tree__item-label slds-truncate" v-bind:title="tocItem.Name"  
                                v-on:click.self.stop="setSelectedtocItem(tocItem.Id)">{{ tocItem.Name }}</span>    
                        </span>
                    </div>
                    <ul v-if="tocItem.hasChildren" v-bind:class="{'slds-hide':!tocItem.expanded,'slds-show':tocItem.expanded}" role="group">
                        <li v-for="tocChildItem in tocItem.children" v-bind:key="tocChildItem.Id" aria-level="2" v-bind:aria-selected="tocChildItem.selected"
                            role="treeitem">
                            <div class="slds-tree__item">
                                ...
                                <span class="slds-has-flexi-truncate">
                                    <span class="slds-tree__item-label slds-truncate" v-bind:title="tocChildItem.Name"
                                        v-on:click.self.stop="setSelectedtocItem(tocChildItem.Id)">{{ tocChildItem.Name }}</span>
                                </span>
                            </div>
                        </li>
                    </ul>
